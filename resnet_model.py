...
def conv2d_fixed_padding(inputs, filters, kernel_size, strides, data_format):
  ...
  if strides > 1:
      ...
      init_weight = tf.truncated_normal_initializer(stddev=math.sqrt(4.0 / (kernel_size * kernel_size * filters)))
  else:
      init_weight = tf.truncated_normal_initializer(stddev=math.sqrt(2.0 / (kernel_size * kernel_size * filters)))
...
def cifar10_resnet_v2_generator(resnet_size, num_classes, data_format=None):
    ...
    def model(inputs, scope, is_training, _reuse):
        with tf.variable_scope(scope, reuse=_reuse):
            ...
            inputs = tf.layers.conv2d(
                ...
                kernel_initializer=tf.truncated_normal_initializer(stddev=math.sqrt(1.0 / (3 * 3 * 3))),
                data_format=data_format, name='firstconv2d')
